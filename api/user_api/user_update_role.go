...
func (UserApi) UserUpdateRoleView(context *gin.Context) {
	var cr UserRole
	...
	if err != nil {
		res.FailWithMessage("用户id错误，用户不存在", context)
		return
	}
	...
	if err != nil {
		...
		res.FailWithMessage("修改权限失败", context)
		return
	}
	res.OkWithMessage("修改权限成功", context)
}
