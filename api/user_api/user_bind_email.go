...
func (UserApi) UserBindEmailView(context *gin.Context) {
	...
	if cr.Code == nil {
		...
		if err != nil {
			...
			res.FailWithMessage("session错误", context)
			return
		}
		...
		res.OkWithMessage("验证码已发送，请查收", context)
		return
	}
	// 第二次，用户输入邮箱，验证码，密码
	...
	// 校验验证码
	if code != *cr.Code {
		res.FailWithMessage("验证码错误", context)
		return
	}
	// 修改用户的邮箱
	...
	if len(cr.Password) < 4 {
		res.FailWithMessage("密码强度太低", context)
		return
	}
	...
	if err != nil {
		global.Log.Error(err)
		res.FailWithMessage("绑定邮箱失败", context)
		return
	}
	// 完成绑定
	res.OkWithMessage("邮箱绑定成功", context)
	return
}
