...
func (UserApi) UserCreateView(context *gin.Context) {
	var cr UserCreateRequest
	if err := context.ShouldBindJSON(&cr); err != nil {
		res.FailWithError(err, &cr, context)
		return
	}
	err := user_ser.UserService{}.CreateUser(cr.UserName, cr.NickName, cr.Password, cr.Role, "", context.ClientIP())
	if err != nil {
		global.Log.Error(err)
		res.FailWithMessage(err.Error(), context)
		return
	}
	res.OkWithMessage(fmt.Sprintf("用户%s创建成功!", cr.UserName), context)
	return
}
