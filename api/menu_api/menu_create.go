...
func (MenuApi) MenuCreateView(context *gin.Context) {
	var cr MenuRequest
	...
	if count > 0 {
		res.FailWithMessage("重复的菜单", context)
		return
	}
	...
	if err != nil {
		...
		res.FailWithMessage("菜单添加失败", context)
		return
	}
	if len(cr.ImageSortList) == 0 {
		res.OkWithMessage("菜单添加成功", context)
		return
	}
	...
	if err != nil {
		global.Log.Error(err)
		res.FailWithMessage("菜单图片关联失败", context)
		return
	}
	res.OkWithMessage("菜单添加成功", context)
