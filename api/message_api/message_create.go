...
func (MessageApi) MessageCreateView(context *gin.Context) {
	// 当前用户发布消息
	...
	if err != nil {
		res.FailWithMessage("发送人不存在", context)
		return
	}
	err = global.DB.Take(&recvUser, cr.RevUserID).Error
	if err != nil {
		res.FailWithMessage("接收人不存在", context)
		return
	}
	...
	if err != nil {
		...
		res.FailWithMessage("消息发送失败", context)
		return
	}
	res.OkWithMessage("消息发送成功", context)
	return
}
